...
>(({ className, hideIcon = false, payload, verticalAlign = 'bottom', nameKey, ...props }, ref) => {
  ...
  return (
    ...
    >
      {payload.map((item) => {
        // Para Pie charts con nameKey, el valor del campo nameKey está en item.payload[nameKey]
        // item.value contiene el valor numérico del dataKey
        let key: string;
        if (nameKey && item.payload) {
          // Para Pie charts: usar el valor del campo nameKey del payload
          key = `${(item.payload as Record<string, unknown>)[nameKey] || item.dataKey || 'value'}`;
        } else {
          // Para otros charts: usar item.dataKey
