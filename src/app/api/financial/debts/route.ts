...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json<ApiResponse>({ success: false, error: 'Error al obtener deudas' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json<ApiResponse>({ success: false, error: 'Error al crear deuda' }, { status: 500 });
  }
}
